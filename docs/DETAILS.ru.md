...
Этот результат подтверждает две вещи. Node разбирает workflow внутри async-обертки, а в исходнике остались обязательные маркеры observability, DAG, CTX-gate, schema, агентов и раскладки репозитория. Самих агентов smoke не запускает.
...
| Cast | Роли и проверенные биографии ученых для персон агентов |
...
Финальный ответ получает `status: "done"`, только если испытатель вернул `ГОТОВА`. При другом вердикте pipeline отдает `status: "needs_fix"` и список найденных проблем.
...
- Сеть: Cast просит host проверить биографические факты через веб. Zeuz не задает сетевую политику.
- Секреты: репозиторию не нужны credentials. Не клади секреты в спецификацию и создаваемые артефакты.
- Телеметрия: удаленного telemetry client нет. Runtime snapshots остаются в локальном JSONL.
...
| `agents/` | Prompts контролера и шести специалистов |
