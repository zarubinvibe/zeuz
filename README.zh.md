...
Zeuz 按你的规格搭出多智能体系统：角色、阶段、闸门，以及一个说真话的测试者。
...
Zeuz 是一座工厂。你拿来一份完整规格，它照着搭出系统：分配角色、排出阶段、在不可逆的地方放上闸门，最后交给测试者看。你拿到的是一个项目目录。不是服务，不是一个运行按钮，是一个能读的目录。

## 它解决什么问题

每做一套新的智能体系统，都要从头再想一遍：角色、阶段、日志、闸门。最先被忘掉的总是闸门，因为它碍事。Zeuz 替你把这套结构守住，不许把碍事的那一半扔掉。

## 最大的优势

**最大的优势：** 不可逆的步骤必须挡在一道看得见的检查后面。

**为什么这样更好：** 架构和构建的提示词都这么要求，而测试者在下结论之前会去看这些闸门是不是真的在。只有它点头，这次运行才报告完成。

## 工作流程

流水线在你的宿主里按阶段走。每个阶段有自己的角色、输入，以及一件你能打开来读的产物。
...
需要 Bash 和 Node.js。这个检查不调用任何模型：它只看工厂的源码是否完整。
...
**你会得到：** 检查以一行通过的闸门结束：Node 能解析流水线，必需的标记都还在。
...
信任边界和上报方式写在 [SECURITY.md](SECURITY.md)。

## 局限

状态：参考实现。源码、提示词、规则和一个静态检查。没有运行器。
...
想更深：[完整参考](docs/DETAILS.md)、[路线图](specs/00-roadmap.md)、[构建规则](rules/best-practices.md)、[决策记录](docs/decisions/)。
